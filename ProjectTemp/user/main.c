...
--------------------------------------------------------------------------------
*  µ —È √˚       : ADC◊™ªª µ—È
*  µ—ÈÀµ√˜       : Õ®π˝printf¥Ú”°ADºÏ≤‚ ˝æ› £¨µ˜Ω⁄ADƒ£øÈ≈‘±ﬂµƒµÁŒª∆˜ ‘⁄¥Æø⁄÷˙ ÷…œº¥ø… ‰≥ˆµÁ—π£¨
                    º˚Œƒº˛ƒ⁄ΩÿÕº
* ¡¨Ω”∑Ω Ω       :
* ◊¢    “‚       :  À˘”√∫Ø ˝‘⁄Õ∑Œƒº˛.cŒƒº˛ƒ⁄
*******************************************************************************/
...
extern uint16_t ADC_ConvertedValue[10][3];//adc ‰∏âË∑ØÈááÈõÜ
...
int main()
{
    ...
    SystemInit();//≥ı ºªØRCC …Ë÷√œµÕ≥÷˜∆µŒ™72MHZ
    delay_init(72);      //—” ±≥ı ºªØ
    ...
    ///////////////ADC Initial///////////////////ADCƒ£øÈ π”√¡À»˝∏ˆ∆¨…œAD PC0 PC1 PC2  ∑÷± « AD10 11 12 Õ®µ¿
    ...
    /////////////////Serial port initial//////////////”√¿¥Ω¯––¥Æø⁄Õ®—∂µƒ£¨printf  µœ÷¡À÷ÿ∂®œÚ
    printf_init(); //printf≥ı ºªØ

    ///////////////DAC Initial /////////////////DAC ≤ø∑÷µƒ≥ı ºªØ£¨Œ“√«≤…”√µƒ «DAC1  “≤æÕ «PA4 ø⁄µƒ ‰≥ˆ
    dma_test();                //DAC≥ı ºªØ
    // DAC_SetChannel1Data(DAC_Align_12b_R, 0);//≥ı º÷µŒ™0


    ///////////////Valve Status Initial/////////////

    ValveState_Init(); //π©∆¯∑ß√≈GPIOø⁄µƒ≤ø∑÷≥ı ºªØ



    ////////////////LCD Intial ///////////////////
    LCD_Init();    //“∫æß∆¡≥ı ºªØ
    GridLayer();  //œ‘ æ∆¡µƒÕ¯¬ÁøÚº‹≤„
    Gas_StateLayer();//œ‘ æ∆¡ π©∆¯∑ß◊¥Ã¨≥ı ºªØ≤ø∑÷
    ...
    Àµ√˜£∫  PC
                            IP£∫192.168.1.199
                            ∂Àø⁄∫≈£∫4001 Œ¥∂®£¨Õ®π˝≤‚ ‘—°”√ ≤√¥∂Àø⁄∫≈∂ºƒ‹πª µœ÷

                            ≤‚ ‘µƒ ±∫Úµƒƒø±ÍµƒIP∫Õƒø±Íµƒ∂Àø⁄∫≈ π”√µƒæÕ «w5500µƒIP”Î∂Àø⁄∫≈
    ...
    **********************************************/
    ...
    System_Initialization();    //STM32œµÕ≥≥ı ºªØ∫Ø ˝(≥ı ºªØSTM32 ±÷”º∞Õ‚…Ë)
    Load_Net_Parameters();      //◊∞‘ÿÕ¯¬Á≤Œ ˝
...
//   ////////////////////////Exti interrupt /////////////////////////////
    ...
    while(1)
    {
        //printf("while mid");
        //      LED2=1;
        //  GPIO_SetBits(GPIOC,GPIO_Pin_14);     //IOø⁄ ‰≥ˆ∏ﬂµÁ∆Ω
        //  delay_ms(100);
        //      GPIO_ResetBits(GPIOC,GPIO_Pin_14); //IOø⁄ ‰≥ˆµÕµÁ∆Ω
        /*
        //LED12=0; //program running
        GPIO_SetBits(GPIOA,GPIO_Pin_11);     //IOø⁄ ‰≥ˆ∏ﬂµÁ∆Ω
        //LED11=0; //program running
        GPIO_SetBits(GPIOC,GPIO_Pin_6);  //IOø⁄ ‰≥ˆ∏ﬂµÁ∆Ω
        LED10=0; //program running
        GPIO_SetBits(GPIOC,GPIO_Pin_12);     //IOø⁄ ‰≥ˆ∏ﬂµÁ∆Ω


        delay_ms(100);
        GPIO_ResetBits(GPIOA,GPIO_Pin_11);   //IOø⁄ ‰≥ˆ∏ﬂµÁ∆Ω
        //LED11=0; //program running
        GPIO_ResetBits(GPIOC,GPIO_Pin_6);  //IOø⁄ ‰≥ˆ∏ﬂµÁ∆Ω
        //LED10=0; //program running
        GPIO_ResetBits(GPIOC,GPIO_Pin_12);   //IOø⁄ ‰≥ˆ∏ﬂµÁ∆Ω
...
//
        ...
        /****************************************Õ¯¬Á¥¶¿ÌÕ®—∂¥¶¿Ì********************************************/
        if(W5500_Interrupt)//¥¶¿ÌW5500÷–∂œ
        {
            //LED0      ÷∏ æµƒ «Õ¯¬Á≤ø∑÷µƒ–≈∫≈µƒ¥´ ‰
            W5500_Interrupt_Process();//W5500÷–∂œ¥¶¿Ì≥Ã–ÚøÚº‹
        }
        if((S0_Data & S_RECEIVE) == S_RECEIVE)//  this is flag to tell if we receive the data
        {
            ...
            Process_Package_Receive();//W5500Ω” ’≤¢∑¢ÀÕΩ” ’µΩµƒ ˝æ›
        ...
        if(Normal_Debug_RunningMode==0x00)
        {
            ...
            else //Open command
            {
                ...
                if (PEV_1479A_ControlMode==0x00) //PEV control Mode 1
                {
                    ...
                    //PSÔºöActually we should not set the pressure to zero,
        ...
        if (sendcount<=40000)
        {
            ...
            Write_SOCK_Data_Buffer(0, Tx_Buffer,4);//ÊåáÂÆöSocket(0~7)ÂèëÈÄÅÊï∞ÊçÆÂ§ÑÁêÜ,Á´ØÂè£0ÂèëÈÄÅ23Â≠óËäÇÊï∞ÊçÆ

            //memcpy(Tx_Buffer, "", 23);
            //Write_SOCK_Data_Buffer(0, Tx_Buffer, 23);//ÊåáÂÆöSocket(0~7)ÂèëÈÄÅÊï∞ÊçÆÂ§ÑÁêÜ,Á´ØÂè£0ÂèëÈÄÅ23Â≠óËäÇÊï∞ÊçÆ

            //Write_SOCK_Data_Buffer(0, Tx_Buffer,a);//ÊåáÂÆöSocket(0~7)ÂèëÈÄÅÊï∞ÊçÆÂ§ÑÁêÜ,Á´ØÂè£0ÂèëÈÄÅ23Â≠óËäÇÊï∞ÊçÆ
        }
        else
        {
            sendcount=0;
...
void Process_Socket_Data(SOCKET s,int Package_Start,int Package_Size)
{
    ...
    char floattest[10];
    char test1[4];
    unsigned short size;//Ω” ’µΩµƒbuffer µƒ¥Û–°
    ////////
    ///////////////Valve Gas Part///////

    u8 ValveValue_Set[12];  //¿¥◊‘Õ¯¬Á¥´»Îπ˝¿¥µƒ–≈∫≈
...
//   printf(Rx_Buffer);
    ...
    //πÿ”⁄RX_Buffer  …œŒªª˙¿Ì¬€…œ ”¶∏√¥´»Îµƒ «“ª¥Æµƒ ˝æ›£¨ ∂¯≤ª «“ª∏ˆµ•¥øµƒŒª
    //ø…“‘ø¥µΩRx_Buffer œ‡µ±”⁄“ª∏ˆ ˝◊È£¨16Œª∞…
    //∂‘”⁄Modbus–≠“È œ»æŸ∏ˆ¿˝◊”
    /*
    ∂¡ ˝æ›£∫    04               03                   02                                                                                            œ¬Œªª˙£∫ªÿ∏¥∆‰–Ë“™∂¡µƒ–≈œ¢ 04 03 02 length data1 data2 *** crc
                    ¥”ª˙µÿ÷∑          π¶ƒ‹¬Î∂¡                      ºƒ¥Ê∆˜µÿ÷∑  ---------------------------------------°∑
    –¥ ˝æ›£∫    04               06                   02             data1  data2 ***  crc16                œ¬Œªª˙£∫ªÿ∏¥∆‰–Ë“™–¥µƒ–≈œ¢ 04 06 02  ****     £®PS£¨’‚∏ˆµÿ∑Ωªÿ∏¥µƒ «”Î…œŒªª˙œ‡Õ¨µƒ∂´Œ˜£©
                    ¥”ª˙µÿ÷∑          π¶ƒ‹¬Î–¥                      ºƒ¥Ê∆˜µÿ÷∑
    ÷√ŒªOn/off£∫  04               05                   02              data1 data2 ***  crc16                   œ¬Œªª˙£∫    ’‚∏ˆ «∑Ò–Ë“™∑µªÿ ˝æ›£ø
                    ¥”ª˙µÿ÷∑          π¶ƒ‹¬Î÷√Œª                    ºƒ¥Ê∆˜µÿ÷∑
    ±®¥Ì–≈œ¢£∫  04               04                  02              data1 data2 ***  crc16            ’‚∏ˆ «œ¬Œªª˙∂‘…œŒªª˙µƒ∑¢ÀÕµƒ ˝æ›”–≤ªÕ¨“‚º˚≤˙…˙µƒ¥ÌŒÛ
                    ¥”ª˙µÿ÷∑          π¶ƒ‹¬Î÷√Œª                    ºƒ¥Ê∆˜µÿ÷∑
    ...
    */
    ...
    if (Rx_Buffer[0+Package_Start]==0x05) //Slave address 0x05
    {
        ...
        {
            case 0x03:    //∂¡»°π¶ƒ‹¬Îºƒ¥Ê∆˜µƒ◊¥Ã¨ -- «∑ÒŒ™∂¡ ˝æ›π¶ƒ‹¬Î
                ...
                switch  (Rx_Buffer[2+Package_Start])
                {
                    ...
                    case 0x01:   //Read Gas 1479A flow meter value
                        ...
                        //AD_Voltage_Status[0 1 2]   ∑÷±±Ì æ1479A 627D 025d
            ...
            case 0x05:    //–¥»Îºƒ¥Ê∆˜µƒ◊¥Ã¨
                //…Ë∂®12π‚¬∑°¢…Ë∂®’Êø’∂»°¢…Ë∂®¡˜¡ø÷µ
                //…Ë∂®∑Ω∞∏£∫Õ®π˝»´æ÷±‰¡ø¥´ÀÕ≥ˆ»•£¨ªπ «◊Ó∫Û÷±Ω”µ˜”√œ‡πÿµƒ∫Ø ˝ƒÿ£ø
                //‘›∂®∑Ω∞∏£∫ ÷±Ω”µ˜”√œ‡πÿµƒ∫Ø ˝
                //’‚∏ˆ «Rx_Bufferµƒ∂®“Â£∫unsigned char Rx_Buffer[2048]


                ////////////////////π©∆¯ƒ£ Ω////////////////////
                /*********************************************
                *π©∆¯ƒ£ Ωµ√—°‘Ò”–2÷÷
                * ƒ£ Ω1£∫…œŒªª˙÷±Ω”øÿ÷∆¡˜¡øº∆ £¨ √ø¥Œ∏¯¡˜¡øº∆…Ë∂®µ±«∞µ√ ˝÷µ
                * ƒ£ Ω2£∫…œŒªª˙≤ªøÿ÷∆¡˜¡ø£¨»√∆‰Õ®π˝◊‘∂Øµ√±’ª∑Ω¯––øÿ÷∆
                * µœ÷µ√∑Ω∞∏
                *
                *
                *********************************************/
                ...
                switch  (Rx_Buffer[2+Package_Start])
                {
                    ...
                    case 0x08:  //Command Mode or timing trigger mode
                        ...
                        if(Rx_Buffer[4+Package_Start]==0x01) //±Ì æπÿ±’µ±«∞≈Á≥ˆƒ£ Ω x
                        {
            ...
            case 0x06:
                ...
                switch(Rx_Buffer[2+Package_Start])
                {
                    ...
                    case 0x0B:  //1479A Gas  Flow value set
                        ...
                        //we should set a critical value to limit the dataÔºå
                        //if it doesn't meet our requirements,we need to send the data error to the pc
    ...
    /*if (Rx_Buffer[0]==0x11)   //≈–∂œºƒ¥Ê∆˜÷–µƒƒ⁄»›æÕÕ®π˝’‚∏ˆ¿¥Ω¯––°£
    {
...
void Cavity_Pressure_SendBack()
{
    ...
    Tx_Buffer[0]=0x05; // ±æª˙µÿ÷∑
    Tx_Buffer[1]=0x03;//π¶ƒ‹√¸¡Ó¬Î
    Tx_Buffer[2]=0x01;//ºƒ¥Ê∆˜µÿ
...
void Status_Register_Update_DMA()
{
    ...
    char *ValveValue_Status;
    float ADC_Value[3];//Áî®Êù•‰øùÂ≠òÁªèËøáËΩ¨Êç¢ÂæóÂà∞ÁöÑÁîµÂéãÂÄº
    int sum;
    u8 i,j;

    /*
        ValveValue_Status=Gas_State_Read(); //∫Ø ˝ µœ÷∂¡»°IOø⁄µƒ∏ﬂµÕµÁ∆Ω÷µ
    ...
    */
    ...
    for(i=0; i<3; i++)
    {
        ...
        AD_Voltage_Status[i]=(float)sum/(10*4096)*3.3;//Ê±ÇÂπ≥ÂùáÂÄºÂπ∂ËΩ¨Êç¢ÊàêÁîµÂéãÂÄº
